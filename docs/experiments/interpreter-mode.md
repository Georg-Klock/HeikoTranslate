...
**Where the code is.** This page is on `main`; the code it describes is not.
The branch was archived as the tag **`archive/interpreter-mode`** (commit
`45e9c15`) and then deleted, so every tool and test named below is reachable
with `git checkout archive/interpreter-mode` and nowhere else. Both experiment
flags (`INTERPRETER_MODE`, `CAPTURE_TURN_AUDIO`) live in the gitignored
`Secrets.plist` and are off; a build from a clean checkout is the shipping
app.
